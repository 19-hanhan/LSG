...
class cached_ifstream
{
  public:
    ...
    void skip(uint64_t n_bytes)
    {
        if (n_bytes <= (cache_size - cur_off))
        {
            // Skip within the cache
            cur_off += n_bytes;
        }
        else
        {
            // Skip past the cache and adjust the file stream
            uint64_t cached_bytes = cache_size - cur_off;
            reader.seekg(n_bytes - cached_bytes, std::ios::cur);
            ...
            {
                cur_off = cache_size; // Indicate that all future reads should be from file
            }
        }
    }
