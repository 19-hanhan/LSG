...
// hanhan add 通过location批量计算距离
template <typename data_t>
void PQDataStore<data_t>::get_distance(const location_t location, const std::vector<location_t> &ids,
                            std::vector<float> &distances, AbstractScratch<data_t> *scratch_space) const 
{
    throw diskann::ANNException("This feature has not yet been developed.", -1);
}
