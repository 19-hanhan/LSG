...
/**
 * 根据给定的参数构建Vamana索引
 * 首先尝试一次性构建索引，如果内存不足，则通过分片和合并的方法来构建索引
 * 这个函数处理不同的度量和过滤标签，并且在构建索引时考虑了内存预算限制
*/
                              ...
                              const uint32_t Lf)
{
    /**
     * 获取元数据
     * 使用 diskann::get_bin_metadata 从 base_file 中读取数据集的元数据，包括样本数量和维度
    */
    ...
    /**
     * 估计内存使用
     * 使用 estimate_ram_usage 估计构建全索引所需的内存量，并与内存预算进行比较
     * 如果内存足够，则一次性构建索引
    */
    ...
    */
    ...
    //     else {
    //         // 创建索引对象并构建索引，如果启用了过滤，则调用 build_filtered_index
    //         if (universal_label != "")
    ...
    */
    ...
    // 使用 partition_with_ram_budget 根据内存预算将数据集分成多个部分
    Timer timer;
    int num_parts =
    ...
    for (int p = 0; p < num_parts; p++)
    {
        ...
        if (!use_filters) {
            ...
            // 创建索引对象并构建索引。如果启用了过滤，提取分片标签并调用 build_filtered_index
    ...
    timer.reset();
    // 使用 diskann::merge_shards 合并分片索引，生成最终的合并索引
    ...
    // delete tempFiles
    // 删除分片过程中创建的临时文件，包括分片数据文件、分片ID文件、分片标签文件和分片索引文件
    for (int p = 0; p < num_parts; p++)
    {
        std::string shard_base_file = merged_index_prefix + "_subshard-" + std::to_string(p) + ".bin";
                     ...
                     const uint32_t Lf)
{
    ...
    */
    ...
    /**
     * 度量和数据类型验证
     * 确保对于某些度量（如内积、余弦），数据类型 T 必须是 float
    */
    ...
    */
    ...
    /**
     * 路径和文件名设置
     * 使用提供的路径和前缀构建各种中间文件和最终文件的路径和文件名
    */
    ...
    /**
     * 度量的数据预处理
     * 根据比较度量，函数可能会预处理数据（例如，对余弦度量进行归一化或对内积度量进行变换）。
    */
    ...
    */
     ...
     * 如果启用了过滤，数据和标签可能会被扩充
     * 读取二进制数据文件的元数据
     * 如果使用基于磁盘的产品量化（PQ），则生成量化数据
     * 调用 build_merged_vamana_index 构建索引
    */
    ...
    // 将输入数据量化：采样一部分数据进行聚类并将编码、码本等数据存储到外存上
...
// This code should only be available in the "build" DLL.
    ...
    /**
     * 磁盘布局创建
     * 根据是否使用磁盘 PQ 以及是否需要数据重新排序，调用 create_disk_layout 生成最终的磁盘布局
    */
    timer.reset();
    if (!use_disk_pq)
    {
