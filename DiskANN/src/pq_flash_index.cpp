                                               ...
                                               const bool shuffle)
{
    /**
     * 随机数生成器初始化
     * 使用 std::random_device 和 std::mt19937 初始化随机数生成器，以便在后续步骤中用于打乱节点顺序。
    */
    ...
    /**
     * 确定要缓存的节点数量
     * 确保缓存的节点数量不超过总节点数量的10%，否则调整为10%的数量。
    */
    ...
    /**
     * 初始化 BFS 层次结构
     * 使用两个 tsl::robin_set 集合来表示当前层和前一层的节点集合。
    */
    ...
    /**
     * 添加初始节点
     * 将 medoids 节点加入 cur_level 集合，作为 BFS 的起点。
    */
    ...
    */
    ...
    {
        // 每一层的 BFS 开始时，将 prev_level 和 cur_level 交换，并清空 cur_level。
        ...
        // 从 prev_level 中选择节点，检查是否已经在 node_set 中，如果不在则加入，并添加到 nodes_to_expand。
        ...
        // 根据 shuffle 标志决定是否打乱 nodes_to_expand。
        if (shuffle)
...
/**
 * gpt:
 * 这段代码实现了 PQFlashIndex 类的 cached_beam_search 方法，
 * 该方法用于在大规模近似最近邻（ANN）搜索中执行基于光束搜索的查询。
 * 它结合了基于产品量化（PQ）的距离计算和基于磁盘的节点访问，
 * 以在大型数据集上进行高效的近似最近邻搜索。以下是代码的主要步骤和功能解释：
*/
template <typename T, typename LabelT>
void PQFlashIndex<T, LabelT>::cached_beam_search(const T *query1, const uint64_t k_search, const uint64_t l_search,
                                                uint64_t *indices, float *distances, const uint64_t beam_width,
                                                 ...
                                                 QueryStats *stats)
{
    ...
    */
    ...
    /**
     * 规范化步骤。对于余弦，我们只需对 mips 查询进行规范化，
     * 对前 d-1 个 dim 进行规范化，并为最后一个 dim 添加 0，
     * 因为使用了额外的坐标将 MIPS 转换为 L2 搜索
    */
