...
/**
 * 给定 train_data 中维度为 num_train * dim 的训练数据，使用 k-means 算法生成 PQ 枢轴，
 * 将坐标划分为 num_pq_chunks（如果它划分维度，否则四舍五入）块，并在每个块中运行 k-means 来计算 PQ 枢轴，
 * 并将其以 bin 格式存储在文件 pq_pivots_path 中，作为 s num_centers*dim 浮点二进制文件
*/
