    ...
    const std::vector<LabelT> &filter_labels, bool search_invocation)
{
    /**
     * 初始化变量和数据结构
     * 初始化存储扩展节点、最佳候选节点和已访问节点的数据结构
    */
    std::vector<Neighbor> &expanded_nodes = scratch->pool(); // 领居结果集result
     ...
     * 对齐查询向量，并预处理查询数据
    */
    location_t location = scratch->get_location();
    ...
    /**
     * 选择使用哪种数据结构来标记访问过的节点
     * 根据节点总数决定使用 bitset 还是 robin set 来标记访问过的节点
    */
    ...
    */
    ...
    auto compute_dists = [this, scratch, pq_dists](const std::vector<uint32_t> &ids, std::vector<float> &dists_out) {
        if (~scratch->get_location())  {
            _pq_data_store->get_distance(scratch->get_location(), ids, dists_out, scratch);
        } else {
    ...
    /**
     * 初始化候选池，将初始节点添加到候选池中，并标记为已访问
    */
    // Initialize the candidate pool with starting points
    // 将init_ids的节点保存到best_L_nodes中
    for (auto id : init_ids)
    {
        ...
        if (is_not_visited(id))
        {
            ...
            if (~location)  {
                distance = _pq_data_store->get_distance(location, id);
            } else {
                distance = _pq_data_store->get_distance(aligned_query, id);
            }
            // if (distance != distance1) printf("query_dis:%.2f, location_dis:%.2f\n", distance, distance1);
    ...
    /**
     * 迭代扩展节点，循环处理候选节点，将未扩展的节点添加到扩展节点池中。
    */
    uint32_t hops = 0;
    uint32_t cmps = 0;
    while (best_L_nodes.has_unexpanded_node())
    {
        ...
        // 将节点添加到扩展节点(expanded_nodes->pool)中以创建稍后剪枝的池(剪枝用的数组)
        if (!search_invocation)
        {
            if (!use_filter)
            {
            ...
            { // in filter based indexing, the same point might invoke
                ...
                // 在基于过滤器的索引中，同一点可能会调用
                // 多个iterate_to_fixed_points，所以需要小心
                // 不要多次将相同的项目添加到池中。
                                                        ...
                                                        uint32_t filteredLindex)
{
    ...
    if (!use_filter)
    {   
        // 当前点的向量放到scratch->aligned_query()中
        _data_store->get_vector(location, scratch->aligned_query());
        scratch->set_location(location); // 设置id，用于计算ls距离
        // 获取领居
                                          ...
                                          const tsl::robin_set<uint32_t> *const delete_set_ptr)
{
    ...
    */
    ...
    // 确保候选池已经按距离排序并且结果列表为空
    assert(std::is_sorted(pool.begin(), pool.end()));
    assert(result.size() == 0);
    // 如果候选池大小超过 maxc，则截断池
    if (pool.size() > maxc) 
        pool.resize(maxc);
    // 初始化临时空间 occlude_factor，并将其填充为0
     ...
     * 修剪过程
    */
    // 初始 cur_alpha 为 1，循环增加 cur_alpha 直到超过 alpha 或结果列表长度达到 degree。
    float cur_alpha = 1;
    while (cur_alpha <= alpha && result.size() < degree)
    {
        ...
        for (auto iter = pool.begin(); result.size() < degree && iter != pool.end(); ++iter)
        {
            ...
            for (auto iter2 = iter + 1; iter2 != pool.end(); iter2++)
            {
                ...
                {   
                    // occlude_factor存储的值: >1表示邻边q-o1更大, <1表示斜边o1-o2更大
                                             ...
                                             std::vector<uint32_t> &pruned_list, InMemQueryScratch<T> *scratch)
{
    ...
    */
    ...
    /**
     * 如果使用了PQ，则更新每个候选邻居节点与当前节点之间的实际距离
    */
    ...
    */
    ...
    // 根据查询距离对池进行排序，并使用 occlude_list 对其进行修剪
    std::sort(pool.begin(), pool.end());
    pruned_list.clear();
    pruned_list.reserve(range);

    /**
     * 使用 occlude_list 函数根据给定的参数修剪候选邻居节点池，将结果存储在 pruned_list 中
    */
    ...
    /**
     * 饱和图处理
     * 如果启用了饱和图选项且 alpha 大于 1，则进一步处理候选邻居节点池
     * 在不超过指定范围的情况下，将不在 pruned_list 中且不等于当前节点的位置的节点添加到 pruned_list 中
    */
...
template <typename T, typename TagT, typename LabelT> void Index<T, TagT, LabelT>::link()
{
    ...
    */
    ...
    /**
     * 初始化 visit_order 向量，存储所有节点的顺序
    */
    /* visit_order is a vector that is initialized to the entire graph */
    // visit_order 是一个向量，初始化为整个图
    ...
    */
    ...
    /**
     * 设置起始点，如果有冻结点，则第一个冻结点为起始点；否则，通过 calculate_entry_point 计算
    */
    // if there are frozen points, the first such one is set to be the _start
    // 如果有冻结点，则将第一个这样的点设置为 _start
    if (_num_frozen_pts > 0)
        _start = (uint32_t)_max_points;
    else
...
#pragma omp parallel for schedule(dynamic, 2048)
    for (int64_t node_ctr = 0; node_ctr < (int64_t)(visit_order.size()); node_ctr++)
    {
        ...
        */
        ...
        // 添加反向边：pruned_list->node
        inter_insert(node, pruned_list, scratch);

        /**
         * 每处理 100,000 个节点，输出进度信息
        */
...
#pragma omp parallel for schedule(dynamic, 2048)
    for (int64_t node_ctr = 0; node_ctr < (int64_t)(visit_order.size()); node_ctr++)
    {
        auto node = visit_order[node_ctr];
        /**
         * 如果节点的邻居数超过 _indexingRange，则重新修剪邻居列表
        */
...
void Index<T, TagT, LabelT>::build_with_data_populated(const std::vector<TagT> &tags)
{
    ...
    */
    ...
    /**
     * 标签检查
     * 如果启用了标签（_enable_tags），但标签向量的大小与 _nd 不一致，抛出异常
     * 如果启用了标签，为每个标签更新 _tag_to_location 和 _location_to_tag 映射
    */
    ...
    */
    ...
    /**
     * 统计每个节点的出度情况
     * 初始化最大值（max）、最小值（min）、总数（total）和计数器（cnt）
     * 遍历每个节点，获取其邻居列表，统计最大、最小和平均出度
     * 输出日志信息，显示索引构建的最大、平均和最小出度，以及出度小于2的节点数量
    */
    ...
    /**
     * 设置 _has_built 为 true，表示索引已构建完成
    */
    _has_built = true;
}
template <typename T, typename TagT, typename LabelT>
...
{
    /**
     * 使用 std::unique_lock 锁定 _update_lock 以确保线程安全
    */
    ...
    /**
     * 错误检查
     * 如果 num_points_to_load 为0，抛出异常
     * 检查数据文件是否存在，如果文件不存在，抛出异常
     * 如果 filename 为 nullptr，抛出异常
    */
    // error checks
    if (num_points_to_load == 0)
        throw ANNException("Do not call build with 0 points", -1, __FUNCSIG__, __FILE__, __LINE__);
    ...
    /**
     * 使用 diskann::get_bin_metadata 从文件中读取数据集的元数据，包括样本数量和维度
    */
    diskann::get_bin_metadata(filename, file_num_points, file_dim);
    ...
    /**
     * 检查文件内容
     * 如果文件中的数据点数量大于索引支持的最大数据点数量，抛出异常
     * 如果请求加载的数据点数量大于文件中的数据点数量，抛出异常
     * 如果文件的维度与索引的维度不匹配，抛出异常
    */
    ...
    /**
     * 向_data_store中填充数据，如果已经分片，filename是现在某一块的数据
    */
    ...
    // 调用 build_with_data_populated(tags) 方法使用已加载的数据和标签构建索引
    build_with_data_populated(tags);
}
...
template <typename T, typename TagT, typename LabelT> void Index<T, TagT, LabelT>::generate_frozen_point()
{
    ...
    // 通过计算 datafopt 的 medoid 来确定图的导航节点
