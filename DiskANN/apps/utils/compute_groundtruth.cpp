...
#pragma omp parallel for schedule(static, 4096)
    ...
    size_t q_batch_size = (1 << 9); // 调整每次多少条数据
...
inline void load_bin_title(const char *filename, size_t &npts, size_t &ndims)
{
    std::ifstream reader;
    reader.exceptions(std::ios::failbit | std::ios::badbit);
    reader.open(filename, std::ios::binary);
...
inline void save_groundtruth_as_one_file_by_part(const std::string& filename, int32_t* data, float* distances, size_t total, size_t npts, size_t ndims, int num_parts) {
    std::string data_dir = filename;
    std::string distance_dir = filename + "_distance";

    // Determine the file open mode based on the part number
    std::ios_base::openmode mode = num_parts ? (std::ios::binary | std::ios::app) : (std::ios::binary | std::ios::out);

    // Open the file with the determined mode
    std::ofstream writer_data(data_dir, mode);
    std::ofstream writer_distance(distance_dir, mode);

    // For the first part, write the npts and ndims at the beginning
    if (!num_parts) {
        int npts_i32 = (int)total, ndims_i32 = (int)ndims;
        writer_data.write((char *)&npts_i32, sizeof(int));
        writer_data.write((char *)&ndims_i32, sizeof(int));
        writer_distance.write((char *)&npts_i32, sizeof(int));
        writer_distance.write((char *)&ndims_i32, sizeof(int));
    }

    // Write data and distances
    writer_data.write((char *)data, npts * ndims * sizeof(uint32_t));
    writer_distance.write((char *)distances, npts * ndims * sizeof(float));

    // close writer
    writer_data.close();
    writer_distance.close();

    // Log the process
    std::cout << "Saving truthset part " << num_parts << " (npts, dim, npts*dim id-matrix, npts*dim dist-matrix) with npts = "
              << npts << ", dim = " << ndims << ", size = " << 2 * npts * ndims * sizeof(uint32_t) + 2 * sizeof(int)
              << "B" << std::endl;
}

inline void merge_groundtruth(const std::string& filename) {
    std::string data_dir = filename;
    std::string distance_dir = filename + "_distance";
    ...
    std::ifstream reader;
    reader.exceptions(std::ios::failbit | std::ios::badbit);
    reader.open(distance_dir, std::ios::binary);
    std::ofstream writer(data_dir, std::ios::binary | std::ios::app);
    ...
    // 分片写入
    int num_parts = (int)std::ceil(1.0 * npts_i32 / PARTSIZE);
    std::cout << "Merge truthset part of " << num_parts << std::endl;
    for (int p = 0; p < num_parts; p++) {
        int num = (std::min)(PARTSIZE, npts_i32 - p * PARTSIZE);
        float* dists = new float[num * dim_i32];
        reader.read((char *)dists, num * dim_i32 * sizeof(float));
        writer.write((char *)dists, num * dim_i32 * sizeof(float));
        std::cout << "\tWrite successful distance part of " << p << std::endl;
        free(dists);
    }
                                                                            ...
                                                                            std::vector<uint32_t> &location_to_tag)
{
    ...
    for (int p = 0; p < num_parts; p++)
    {
        ...
        // 数据被分成了num_parts份（根据被搜索集的内存大小分块）
        // 每条查询在每份中都要找到k个最近邻，这样最后得到的结果就是精确的
        // 查询方式就是暴搜
             ...
             const diskann::Metric &metric, const std::string &tags_file = std::string(""))
{
    ...
    int num_parts = get_num_parts<T>(query_file.c_str());
    size_t num_query, dim_query;
    load_bin_title(query_file.c_str(), num_query, dim_query);

    for (int p = 0; p < num_parts; p++){
        std::cout << "Query search parts: " << p << std::endl;
        size_t npoints, nqueries, dim;
        float *query_data;
        
        load_bin_as_float<T>(query_file.c_str(), query_data, nqueries, dim, p);
        ...
        save_groundtruth_as_one_file_by_part(gt_file, closest_points, dist_closest_points, num_query, nqueries, k, p);
        delete[] closest_points;
        delete[] dist_closest_points;
        diskann::aligned_free(query_data);
    }
    merge_groundtruth(gt_file);

    auto e = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> diff = e - s;
    diskann::cout << "Compute gt time: " << diff.count() << std::endl;
    return 0;
}
