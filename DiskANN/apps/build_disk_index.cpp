...
int main(int argc, char **argv)
{
    ...
    bool use_opq = false;
    std::string knn_path; // hanhan
    float ls_alpha; // hanhan
    ...
    */
    ...
    try
    {
        ...
        // hanhan options
        optional_configs.add_options()("knn_path", po::value<std::string>(&knn_path)->default_value(""),
                                       "Filename of AVGKNN(by sampling)");
        optional_configs.add_options()("ls_alpha,a", po::value<float>(&ls_alpha)->default_value(0.0),
                                       "top of ls");
    ...
    diskann::SET_LS(dist_fn, knn_path, ls_alpha);
    if (dist_fn == std::string("l2") || dist_fn == std::string("ls"))
