                      ...
                      const std::vector<std::string> &query_filters, const bool use_reorder_data = false)
{
    ...
    */
    ...
    /**
     * gpt:这段代码通过 BFS 算法在图中遍历节点，并将遍历到的节点缓存到 node_list 中，
     * 以提高后续查询的效率。通过限制缓存节点的数量，确保不会缓存过多节点。
     * 同时，可以选择是否打乱节点顺序，以提高缓存的随机性。
    */
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    */
    ...
    diskann::cout << std::setw(6) << "L" << std::setw(12) << "Beamwidth" << std::setw(16) << "QPS" << std::setw(16)
                  << "Mean Latency" << std::setw(16) << "99.9 Latency" << std::setw(16) << "Mean Hops" 
    ...
    /**
     * Lvec包含所有选定的search_list，也就是efs=10,20,30...
    */
    for (uint32_t test_id = 0; test_id < Lvec.size(); test_id++)
    {
        ...
        /**
         * 将第test_id个查询的结果全部保存到query_result_ids[test_id]中
         * query_result_ids是一个二维数组，每行表示一个L下的查询结果
        */
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        */
        ...
        diskann::cout << std::setw(6) << L << std::setw(12) << optimized_beamwidth << std::setw(16) << qps
                      << std::setw(16) << mean_latency << std::setw(16) << latency_999 << std::setw(16) << mean_hops
                      << std::setw(16) << mean_ios << std::setw(16) << mean_cpuus;
        if (calc_recall_flag)
        {
    ...
    for (auto L : Lvec)
    {
        if (L < recall_at)
            continue;
        if (!query_result_ids[test_id].size())
            continue;
