...
#pragma omp for
        for (int i = 0; i < ntotal; i++) {
            std::vector<int> tmp(S); //初始化一个大小为 S 的近邻池

            gen_random(rng, tmp.data(), S, ntotal); //随机生成初始邻居
...
void NNDescent::build(DistanceComputer& qdis, const int n, bool verbose) {
    ...
    // Clear the old graph
    for (int i = 0; i < ntotal; i++) { //遍历图中所有节点
        std::sort(graph[i].pool.begin(), graph[i].pool.end()); //对每个节点的邻居列表进行排序
        for (int j = 0; j < K; j++) { //遍历节点前 K 个邻居
            FAISS_ASSERT(graph[i].pool[j].id < ntotal); //检测id有效性
            final_graph[i * K + j] = graph[i].pool[j].id; //前 K 个邻居的 ID 存储到 final_graph
        }
    }
