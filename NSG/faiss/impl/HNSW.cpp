  ...
  reduction(+: tot_reciprocal) reduction(+: n_node)
    for (int i = 0; i < levels.size(); i++) {
        if (levels[i] > level) {
            ...
            minn=std::min(minn,neighset.size());
            maxn=std::max(maxn,neighset.size());
...
// user this to shink knn 
void HNSW::shink_reverse_neighbors_2(std::vector<idx_t>& new_neibor,
                DistanceComputer& dis,idx_t cur){
  ...
  for (auto a : new_neibor)
  {
      ...
      input.emplace(dis.symmetric_dis(cur,a), (int)a);
  }
  ...
  // 裁边
  HNSW::shrink_neighbor_list(dis, input, returnlist, m);
  // 重新组建new_neighbor
  new_neibor.resize(0);
  for (int i = 0; i < returnlist.size()&&i<32; ++i)
  {
              ...
              int level)
{
  ...
  size_t i = begin;
  while (resultSet.size()) {
    // std::cout<<resultSet.top().d<<std::endl;
    // 20230812 将多的点去除
    if(resultSet.size() > end-begin){
      resultSet.pop();
      continue;
    }
...
// 如果check_relative_distance == true 不受efsearch影响
int HNSW::search_from_candidates(
    ...
    // （默认 ： true）
    bool do_dis_check = false; // check_relative_distance;
    int nstep = 0;
    ...
    while (candidates.size() > 0) {
        ...
        nstep++;
        //（如果do_dis_check==true,则不会经过当前条件break）
...
std::priority_queue<HNSW::Node> HNSW::search_from_candidate_unbounded_to_gt(
        const Node& node,
        DistanceComputer& qdis,
        int ef, 
        VisitedTable* vt,
        HNSWStats& stats,idx_t gt_idx) const {
    int ndis = 0;
    // 大根堆
    std::priority_queue<Node> top_candidates;
    // 小根堆
    ...
    while (!candidates.empty()) {
        ...
        if (v0 == gt_idx)
        {
            // std::cout<<nstep<<":"<<v0<<":"<<gt_idx<<std::endl;
            findGt=true;
            break;
        }
        ...
        for (size_t j = begin; j < end && j<begin+32; ++j) {
            int v1 = neighbors[j];
    ...
    std::ofstream out("./out_data/20knng4.csv",std::ofstream::app);
    if (findGt)
    {
        out<<nstep<<std::endl;  
    }else{
...
HNSWStats HNSW::search_gt(
        DistanceComputer& qdis,
        int k,
        idx_t* I,
        float* D,
        VisitedTable& vt,long gt_id) const {
    HNSWStats stats;
    ...
    if (upper_beam == 1) {
        ...
        if (search_bounded_queue) {
            ...
            idx_t tmpt=gtvector[gt_id][0];
            std::priority_queue<Node> top_candidates =
                    search_from_candidate_unbounded_to_gt(
                            Node(d_nearest, nearest), qdis, ef, &vt, stats,tmpt);
            // 保证k个点
            while (top_candidates.size() > k) {
                top_candidates.pop();
            }
    ...
    */
    if (gt_dis < 0) {
      ...
      for (int i_best_I = 0; i_best_I < best_I.size(); i_best_I++) {
        // 与当前查询的gt对比
        if ((best_I[i_best_I] == gtvector[gt_idx][0]) && gt_dis < 0) {
            gt_dis = ndis;
            break;
        }
      }
...
// For search_mode = 4.
/*
*  数据分割之后找到的最近邻的点不一定是gt中的第一个
*  因此，需要在访问过程中碰到gt时就停止
*/
void HNSW::search_from_candidate_unbounded_train_binary_datasets(
  const Node& node,
  DistanceComputer& qdis,
  idx_t *I, float *D, int k,
  idx_t gt_idx,
  VisitedTable *vt) const
{
  ...
  while (!candidates.empty()) {
    ...
    for (size_t j = begin; j < end; ++j) {
      ...
      // 查询点数计数
      ++ndis;
      // 当前maxheap_push元素小于k直接加入
      if (nres < k) {
        faiss::maxheap_push(++nres, D, I, d1, v1);
        // 如果当前元素小于堆中最大元素，出堆最大的，进去当前较小元素
      } else if (d1 < D[0]) {
        faiss::maxheap_pop(nres--, D, I);
        faiss::maxheap_push(++nres, D, I, d1, v1);
      } 

      // 这里不可以break，由于最终结果中可能没有该点
      if(v1 == gtvector[gt_idx][0]){
            gt_nstep = nstep;
            gt_dis = ndis;
        }
...
// Customized search() for search_mode = 1, 2, 3,4.
                         ...
                         VisitedTable& vt) const
{
  ...
  if (search_mode == 1) {
    ...
    search_from_candidate_unbounded_train_binary_datasets(Node(d_nearest, nearest),
      qdis, I, D, k, gt_idx, &vt);
