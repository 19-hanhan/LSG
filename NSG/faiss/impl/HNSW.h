...
struct HNSW {
    ...
    void shink_reverse_neighbors_2(std::vector<idx_t>& new_neibor,
    ...
    std::priority_queue<HNSW::Node> search_from_candidate_unbounded_to_gt(
        const Node& node,
        DistanceComputer& qdis,
        int ef, 
        VisitedTable* vt,
...
/*----------------------my_add---------------------*/
      ...
      // For search_mode = 4.
      // Generate training data for prediction-based approach.
      void search_from_candidate_unbounded_train_binary_datasets(
        const Node& node,
        DistanceComputer& qdis,
        idx_t *I, float *D, int k,
        idx_t gt_idx,
