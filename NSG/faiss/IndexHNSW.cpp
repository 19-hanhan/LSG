...
// #pragma omp for
            for(idx_t i = i0; i < i1; i++) {
                ...
                if (search_mode == 0) { // fixed configuration
                    ...
                    // 这里是重排序，不可对结果重排序，如果利用了D[0]
                    //maxheap_reorder (k, simi, idxi);
                }else if (search_mode == 5)
                {
                    hnsw.search_gt(*dis, k, idxi, simi, vt,i);
                } 
                else {
                    // printf("search_mode=1");
...
void IndexHNSW::prund(idx_t n)
{
    ...
    ScopeDeleter1<DistanceComputer> del(dis);
    // knn
    std::vector<std::vector<int>> G(n,std::vector<int>(32,-1));

    /// 统计热点，统计热点个数，非热点个数；统计每个点 热点与热点的距离关系，该点与热点之间的距离关系,分别输出到两张表中
    storage_idx_t f[n];
    memset(f,0,sizeof(f));
    for (int i = 0; i < n; ++i)
    {
    ...
    // 按照入度排序找热点
    std::vector<std::pair<int,storage_idx_t>> tmp;
    for(int i=0;i<n;i++){
    ...
    // 取4%作为热点
    std::unordered_set<storage_idx_t> hubs;
    for(int i=0;i<n*0.04;i++){
        hubs.insert(tmp[i].second);
    }
    ...
    for (int i = 0; i < n; ++i)
    {
        size_t begin,end;
        hnsw.neighbor_range(i,0,&begin,&end);
        std::vector<storage_idx_t> nds_hubs;
        std::vector<storage_idx_t> nds_nohubs;
        size_t j;
        for ( j = begin; j < end; ++j)
        {
            if (hnsw.neighbors[j]<0||hnsw.neighbors[j]>=n)
            ...
            if(hubs.find(v1)==hubs.end())
                nds_nohubs.emplace_back(v1);
            else 
                nds_hubs.emplace_back(v1);
        }

        
        // 计算热点之间的距离
        std::vector<float> hubs_diss;
        int cnt_hubs = nds_hubs.size();
        for(int ii=0;ii<cnt_hubs;ii++){ // 热点与热点距离
            for(int jj=ii+1;jj<cnt_hubs;jj++){
                float dd = dis->symmetric_dis(nds_hubs[ii],nds_hubs[jj]);
                // std::cout<<nds_hubs[ii]<<"::"<<nds_hubs[jj]<<"::"<<dd<<std::endl;
                hubs_diss.emplace_back(dd);
            }
        }
        std::sort(hubs_diss.begin(),hubs_diss.end());
        // 输出到一行中
        std::ofstream out("./out_data/hubsdiss.csv",std::ofstream::app);
        for (auto a : hubs_diss)
            out<<a<<",";        
        out<<std::endl;
        out.close();
        ...
        // 当前点与他的热点的距离
        std::vector<float> nohubs_hubs_diss;
        int cnt_no_hubs = nds_nohubs.size();
        for(int jj=0;jj<cnt_hubs;jj++){
            float dd = dis->symmetric_dis(i,nds_hubs[jj]);
            nohubs_hubs_diss.emplace_back(dd);
        }
        std::sort(nohubs_hubs_diss.begin(),nohubs_hubs_diss.end());
        // 输出到一行中
        std::ofstream out1("./out_data/nohubsdiss.csv",std::ofstream::app);
        for (auto a : nohubs_hubs_diss)
            out1<<a<<",";        
        out1<<std::endl;
        out1.close();


        std::ofstream out2("./out_data/cnthubs_cntnohubs.csv",std::ofstream::app);
        out2<<cnt_hubs<<","<<cnt_no_hubs<<std::endl;        
        out2.close();
    }
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        for ( j = begin; j < end; ++j)
        {
            ...
            new_neibor.push_back(hnsw.neighbors[j]); 
           // first:与热点距离， second：热点反向邻居
           input.emplace(dis->symmetric_dis(i,hnsw.neighbors[j]), (int)hnsw.neighbors[j]);
        }

        // 找出32近邻
        int ii=0;
        while(!input.empty()&&ii<32){
            G[i][ii]=input.top().id;
            input.pop();
            ii++;
        }
        ...
        // std::cout<<new_neibor.size()<<"\t";

        hnsw.shink_reverse_neighbors_2(new_neibor,*dis,(idx_t)i);

        // std::cout<<new_neibor.size()<<std::endl;

        size_t begin1,end1;
        hnsw.neighbor_range(i,0,&begin1,&end1);
        size_t j1, k = 0;
        int len = new_neibor.size();
        for ( j1 = begin1; j1 < end1 ; ++j1, ++k)
        {
            if(k < len){
                hnsw.neighbors[j1] = new_neibor[k];
            }
            else 
                hnsw.neighbors[j1] = -1;
        }
    ...
    // 将近邻图输出
    // std::ofstream out("./out_data/20_knng4.csv",std::ofstream::app);
    // for(int i=0;i<n;i++){
    //     for (auto a : G[i])
    //     {
    //         out<<a<<",";        
    ...
    // 将裁边图输出
    // std::ofstream out1("./out_data/prund4.csv",std::ofstream::app);
    // // 统计热点
    // storage_idx_t f1=[n];
    ...
    //     hnsw.neighbor_range(i, 0, &begin, &end);
    //     int k=0;
    //     for (size_t j = begin; j < end&& k<32;j++) {
    ...
    // 统计裁边之后的邻居列表中热点和非热点的数量
    std::vector<std::pair<int,storage_idx_t>> tmp1;
    for(int i=0;i<n;i++){
        tmp1.emplace_back(-f1[i],i);
    }

    std::sort(tmp1.begin(),tmp1.end());

    // 取4%作为热点
    std::unordered_set<storage_idx_t> hubs1;
    for(int i=0;i<n*0.04;i++){
        hubs1.insert(tmp1[i].second);
    }

    for (int l = 0; l < n; ++l)
    {
        size_t begin1,end1;
        hnsw.neighbor_range(l,0,&begin1,&end1);
        std::vector<storage_idx_t> nds_hubs1;
        std::vector<storage_idx_t> nds_nohubs1;
        size_t jj;
        for ( jj = begin1; jj < end1&&jj-begin1<32; ++jj)
        {
            if (hnsw.neighbors[jj]<0||hnsw.neighbors[jj]>=n)
            {
                break;
            }
            storage_idx_t v1 = hnsw.neighbors[jj];
            // std::cout<<v1<<std::endl;
            if(hubs1.find(v1)==hubs1.end())
                nds_nohubs1.emplace_back(v1);
            else 
                nds_hubs1.emplace_back(v1);
        }
        std::ofstream out3("./out_data/shink_cnthubs_cntnohubs.csv",std::ofstream::app);
        out3<<(int)nds_hubs1.size()<<","<<(int)nds_nohubs1.size()<<std::endl;        
        out3.close();
    }

    // out_indegrees("./out_data/after_prund_indegrees.csv"); // 输出结点入度
    // aod_level(); // 输出平均出度
}
