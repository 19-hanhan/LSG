...
void Index::set_nicdm_distance(float* x, float y){
    printf("Index:DO NOTING");
}
...
// 用于重建单个存储向量，利用索引还原为原始向量数据，具体实现在子类中
void Index::reconstruct(idx_t, float*) const {
    FAISS_THROW_MSG("reconstruct not implemented for this type of index");
}

// 用于重建多个存储向量，利用索引还原为原始向量数据，具体实现在子类中
