...
void IndexFlat::set_nicdm_distance(float* x, float y){
    AVGDIS = x;
    alpha = y;
    printf("NICDM参数设置成功\n");
}
...
struct NICDMDistanceComputer : DistanceComputer {
    ...
    explicit NICDMDistanceComputer(const IndexFlat& storage, const float* q = nullptr) 
        : d(storage.d),
          nb(storage.ntotal),
          q(q),
          b(storage.xb.data()),
...
DistanceComputer* IndexFlat::get_distance_computer() const {
    ...
    } else if (metric_type == METRIC_NICDM) {
        return new NICDMDistanceComputer(*this);
    }else {
